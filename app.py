...
try:
    # 檢查模型文件是否存在
    if os.path.exists(LGB_MODEL_FILE) and os.path.exists(XGB_MODEL_FILE):
        lgb_model = joblib.load(LGB_MODEL_FILE)
        xgb_model = joblib.load(XGB_MODEL_FILE)
        model_loaded = True
        print("模型載入成功")
    else:
        print("模型文件不存在，將使用假模型")
        model_loaded = False
        # 創建假模型用於測試
        class DummyModel:
            def predict_proba(self, X):
                # 模擬預測概率: 莊家, 閒家, 和
                return np.array([[0.45, 0.44, 0.11]])
...
def predict():
    try:
        ...
        # 添加观望邏輯 - 如果最高概率不夠高，建議觀望
        if max_prob < 0.5:  # 可以調整這個閾值
            suggestion = "觀望"
            confidence = 0
        else:
        ...
        return jsonify({
            "success": True,
            "model_loaded": model_loaded,  # 添加模型加載狀態
            "predictions": {
                "banker": banker_prob,
                "player": player_prob,
                "tie": tie_prob
